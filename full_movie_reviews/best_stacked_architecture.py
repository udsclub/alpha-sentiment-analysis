...
NAME = "movie_stacked_lstm" + suffix
BOTH_SIDES = False
print(suffix)
...
def model_initialization(embeddings):
    ...
    factors = [1, factor_1, factor_2*factor_1]
    n_factor = 0
    model = Sequential()
    model.add(embeddings)
    model.add(Dropout(0.2))
    for n in range(n_layers - 1):
        model.add(LSTM(int(output / factors[n_factor]), dropout_U=0.1, dropout_W=0.1, return_sequences=True))
        n_factor += 1
    model.add(LSTM(int(output / factors[n_factor]), dropout_U=0.1, dropout_W=0.1))
    model.add(Dropout(0.2))
    model.add(Dense(2))
...
def train_model(optimizer, embeddings, add_to_name):
    ...
    model.fit(X_train, y_train, validation_data=[X_test, y_test], batch_size=2048, nb_epoch=1000,
...
for n_layers in [1, 2, 3]:
    for output in [64, 128, 256]:
        for factor_1 in range(1, min(n_layers+1, 3)):
            for factor_2 in range(1, min(n_layers+1, 3)):
                train_model(Adam(lr=LR), embedding_matrix, "_{}_{}_{}_{}".format(n_layers, output, factor_1, factor_2))
